...
open class JZWeekViewFlowLayout: UICollectionViewFlowLayout {
    ...
    */
    ...
    open func dateForColumnHeader(at indexPath: IndexPath) -> Date {
        ...
        print("TESTTTTTT")
//        let vals = Calendar.current.date(byAdding: .day, value: 1, to: day!)!
        UserDefaults.standard.setValue(true, forKey: "Goday")
        let dateFormatter = DateFormatter()
        dateFormatter.locale = Locale(identifier: "en_US_POSIX")
        dateFormatter.dateFormat = "yyyy-MM-dd"
        let lastWeekDateString = dateFormatter.string(from: day!)
        UserDefaults.standard.setValue(lastWeekDateString, forKeyPath: "Godate")
        print(Calendar.current.startOfDay(for: day!))
