...
- (id)initWithReuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        ...
        locationLabel.font = [SSCalendarEventTableViewCell nameFont];
...
- (void)setEvent:(SSEvent *)event
{
    ...
    if (sep.count > 0 ){
        timeLabel.text = sep[0];
        if (sep.count > 1 ){
            timeLabel.text = [NSString stringWithFormat:@"%@",sep[0]];
            locationLabel.text = [NSString stringWithFormat:@"%@",sep[1]];
        }
        if (sep.count > 2) {
...
- (void)layoutSubviews
{
    ...
    if (_event.location.length > 0)
    {
        ...
        locationLabel.frame = CGRectMake(timeLabel.frame.origin.x, y, width, height);
    }
}
