...
class AccountsListModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [AccountsListResult]!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [AccountsListResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = AccountsListResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [AccountsListResult]
