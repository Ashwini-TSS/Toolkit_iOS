...
class GetPatternUsers : NSObject, NSCoding{

	var responseMessage : String!
	var results : [GetPatternResult]!
	var valid : Bool!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [GetPatternResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = GetPatternResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [GetPatternResult]
