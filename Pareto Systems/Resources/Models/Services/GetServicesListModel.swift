...
class GetServicesListModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [GetServicesListResult]!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [GetServicesListResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = GetServicesListResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [GetServicesListResult]
