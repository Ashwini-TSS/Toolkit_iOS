...
//	ServicesModel.swift
//
//	Create by thabresh thabu on 6/7/2018
...
class ServicesModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [ServicesResult]!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = ServicesResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [ServicesResult]
