...
class GetCompanyListModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [GetCompanyListResult]!
	var valid : Bool!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = GetCompanyListResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [GetCompanyListResult]
