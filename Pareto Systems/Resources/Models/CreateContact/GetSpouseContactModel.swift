//
//	GetSpouseContactModel.swift
//
//	Create by thabresh thabu on 12/7/2018
...
class GetSpouseContactModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [GetSpouseContactResult]!
	var valid : Bool!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [GetSpouseContactResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = GetSpouseContactResult(fromDictionary: dic)
				results.append(value)
			}
            results.sort { $0.lastName < $1.lastName}
//            var sortedArray = results.sorted { $0.firstName.localizedCaseInsensitiveCompare($1) == ComparisonResult.orderedAscending }
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [GetSpouseContactResult]
