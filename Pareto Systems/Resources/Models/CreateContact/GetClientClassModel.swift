...
class GetClientClassModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [GetClientClassResult]!
	var valid : Bool!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [GetClientClassResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = GetClientClassResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [GetClientClassResult]
