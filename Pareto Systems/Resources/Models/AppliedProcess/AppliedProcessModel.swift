...
class AppliedProcessModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [AppliedProcessResult]!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [AppliedProcessResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = AppliedProcessResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [AppliedProcessResult]
