...
class AppointmentTypeModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [AppointmentTypeResult]!
	var valid : Bool!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [AppointmentTypeResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = AppointmentTypeResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [AppointmentTypeResult]
