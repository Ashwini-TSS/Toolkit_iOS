...
class getLinkedRecreationListModel : NSObject, NSCoding{

	var responseMessage : String!
	var results : [getLinkedRecreationResult]!
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [getLinkedRecreationResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = getLinkedRecreationResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [getLinkedRecreationResult]
