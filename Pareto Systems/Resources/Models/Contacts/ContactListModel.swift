...
class ContactListModel : NSObject, NSCoding{
	...
	init(fromDictionary dictionary: NSDictionary){
		...
		results = [ContactListResult]()
		if let resultsArray = dictionary["Results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = ContactListResult(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"Results") as? [ContactListResult]
